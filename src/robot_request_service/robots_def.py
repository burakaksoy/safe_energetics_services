...
#ALL in mm
class abb_irb52(object):
	def __init__(self,R_tool=Ry(np.radians(90)),p_tool=np.array([0.,0,0.0])*1000.,d=0,acc_dict_path=''):
		###ABB IRB 52 1.45 Robot Definition
		self.H=np.concatenate((ez,ey,ey,ex,ey,ex),axis=1)
		p0=np.array([[0],[0],[0.4865]])
		p1=np.array([[0.15],[0],[0]])
		p2=np.array([[0.],[0],[0.7]])
		p3=np.array([[0.6],[0],[0]])   
		p4=np.array([[0],[0],[0]])
		p5=np.array([[0.065],[0],[0]])
		p6=np.array([[0.0],[0],[0.0]])
		...
		###updated range&vel limit
		self.upper_limit=np.radians([180.,150.,65.,200.,115.,400.])
		self.lower_limit=np.radians([-180.,-90.,-245.,-200.,-115.,-400.])
		self.joint_vel_limit=np.radians([180,180,180,320,400,460])
